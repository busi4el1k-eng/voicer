...
// A no-op subscribe for useSyncExternalStore — used only to detect hydration
// (the server snapshot is false, the client snapshot true), never to subscribe.
const subscribeNoop = () => () => {};

// The interactive library browser. `initialVideos` is the server-rendered first
// page (newest first), seeded so the initial HTML already contains real video
// cards (SSR content for crawlers / AdSense) instead of a "Loading…" shell — the
// full library still loads client-side to power filtering / sorting / paging.
export function LibraryBrowser({ initialVideos }: { initialVideos: Video[] }) {
  const { t, locale } = useI18n();
  const router = useRouter();
  // Seeded with the server's first page (see the manifest effect for the full
  // load). null only if the server sent nothing (e.g. a DB hiccup) — then the
  // client falls back to fetching everything and shows the loading state.
  const [manifest, setManifest] = useState<Video[] | null>(
    initialVideos.length ? initialVideos : null,
  );
  // Gate the time-based (trending) sort until the client has hydrated, so the
  // server HTML and the first client render are identical (the seed stays in its
  // given newest-first order); Date.now()-based ranking differs between server
  // and client and would otherwise trip a hydration mismatch. useSyncExternalStore
  // is the SSR-safe hydration signal: false during SSR + the first render, then
  // true — with no setState-in-effect.
  ...
  const sorted = useMemo(() => {
    // Before hydration, keep the server's deterministic newest-first order so the
